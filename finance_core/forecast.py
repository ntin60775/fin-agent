...
class Discrepancy:
    ...
    Факт введён извне и расчёт не заменяет: сверка идёт по канону остатка
    (`deal_balance`) и **может сойтись** — наблюдение, равное расчёту на дату,
    вопросом не становится. Ненулевая разница (наблюдение минус расчёт) значит,
    ...
    if blind:
        return Milestone(
            reason="нехватка до прожиточного минимума не оценена: в месяце "
                   f"{blind[0]} судить нечем — прихода впереди не видно")
...
def _questions(book: Settlements) -> list[Discrepancy]:
    ...
    Один механизм: разница «наблюдение минус расчёт», и вопросом становится
    только её ненулевое значение — сошедшаяся сверка списка не пополняет.
    ...
    книги — обязательный вход в прогноз: без неё такое наблюдение пропало бы
    незамеченным.
    """
