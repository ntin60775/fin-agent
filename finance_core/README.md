...
- **Остаток сделки — один канон: «долг на дату» = тело − движения + начисленное**
  ...
  месяц. **Исключение — участник копилки:** его остаток стоит на теле, пока
  единица не закроется, поэтому равенство для него не заявляется (это
  отложенное «третье расхождение», см. решение о каноне); сам котёл при этом
  считается от тела — проценты в копилку не копятся. Ставки нет — начисления нет
