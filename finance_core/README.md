...
месяца считаются без обещанного — непрошедшее обязательство не отменено. У счёта
есть признак доступности: недоступный в ликвидность не входит, лимита к
использованию не даёт и платёж финансировать не может — с закрытой банком карты не
заплатить, её можно только гасить; платить и переводить с него — ошибка, а не тихий
минус. Неизвестный прожиточный минимум даёт `floor_gap = None` («не оценено»), а не
ноль.
