...
- **Ни остаток, ни сальдо не хранятся.** Обе величины производные: остаток
  считается по канону (условия, движения и начисленное), сальдо — суммой таких
  остатков на дату — дату передаёт вызывающий, «сегодня» движок не спрашивает.
...
- **Сальдо — отчётная величина.** Зачёт не производится: остатки по сделкам
  ...
  минус сколько он мне, и **включает начисленное** — по канону остатка, потому
  что «сколько я должен» один ответ, и это ответ кредитора: проценты, накопленные
  к дате, в нём есть.
...
- **Фактический остаток — наблюдение извне.** `ObservedBalance` — остаток сделки на
  ...
  сверяется с ним. **Сверка идёт по канону и может сойтись:** наблюдение, равное
  расчёту на дату, открытым вопросом не становится и прогноз неполным не делает;
  сверяется и сделка без ставки — её канон равен «тело минус движения» (отдельного
  «сверять нечего» нет, оно остаётся только у регулярного расхода, у которого
  остатка нет вовсе). Ненулевая разница (наблюдение минус расчёт) — не ошибка
  файла, а незакрытый вопрос: правило начисления движка не совпало с правилом
  кредитора — иная ставка, база начисления, начало долга, комиссия. Оно
  закрывается правкой условий сделки или записью факта, а не правкой числа
  (`../docs/decisions/derived-balances.md`). Проверка падает только на
  невозможном (отрицательная сумма, наблюдение по регулярному расходу), а само
  расхождение видно открытым вопросом прогноза (`questions` — список только из
  ненулевых разниц, сошедшихся сверок в нём нет).
...
- **Отдельные строки отчёта.** Требования (`expectations`) в кассу не подмешиваются,
  ...
  вопрос, а не тишина. Вопросом становится только ненулевая разница «наблюдение
  минус расчёт»: сошедшаяся сверка список не пополняет и неполноту не даёт,
  отдельного списка сошедшихся сверок нет.
