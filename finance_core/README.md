...
Требование сравнивается не с сырым остатком события, а с остатком за вычетом
**прожитого** — минимума, уже прожитого с точки отсчёта до даты шага: прожитое
уже израсходовано, но остаток события его не терял. Прожитое считается той же
...
«сегодня» и её не выдумывает. Шаг в самой точке отсчёта прожитого ещё не
имеет — нехватка считается как раньше.

**Точки оценки** в прокате — зеркало дыры: вход каждого месяца (входной остаток
основного кошелька до событий) и шаги основного кошелька. Если приход впереди
виден, месяц без событий получает оценку по входу — число, а не «не оценено».
Приход в самый день входа считается сегодняшним (0 дней впереди): голодать не
перед чем, остаток дня оценивает шаг после прихода.
...
- **Досрочка идёт по стратегии, но сделку можно из неё исключить.**
  ...
  говорит, чью ставку гасить первой. Копилка досрочится вся целиком, поэтому
  запрет любого её участника запрещает и копилку — иначе запрет обошли бы через
  единицу закрытия.
...
- **Отклоняется только невозможное**: сдвиг за горизонт, мост без дыры, мост больше
  доступного лимита кошелька, досрочка больше свободного остатка месяца или остатка
  сделки, конфликт действий внутри варианта. Невыгодное не отклоняется — оно
