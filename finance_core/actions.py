...
список действий, применённый к сценарию **целиком**: петля проката сходится после
всего списка, а не после каждого действия. Сценарий под вариант не переписывается:
`applied` собирает новый вход, база остаётся как была.
...
class Prepay:
    ...
    сделку. У сделки из единицы закрытия деньги уходят в общий котёл: единица
...
def _prepay_facts(book: Settlements, action: Prepay) -> tuple[Movement, Payment]:
    ...
    # Досрочка долговая: кредитным лимитом она не платится — природа платежа
    # названа явно, чтобы это не зависело от значения по умолчанию.
...
def _remaining(book: Settlements, deal: Deal, on: date) -> Decimal:
    ...
    У сделки из единицы закрытия деньги идут в общий котёл, но записываются по
